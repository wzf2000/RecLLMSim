...
本次补充为 SPUR baseline 增加了一个输出分数映射参数，但没有重新调用 LLM。
指标由现有 evaluator/baseline JSONL 输出和复用缓存后的 SPUR 映射输出统计得到。
...
| SPUR-style evaluator (`DSAT -> 3`) | 0.6894 | 0.3422 | 0.7033 | 0.0000 | 0.2063 | 0.8202 | 0/1526/4948 |
| SPUR-style evaluator (`DSAT -> 2`) | 0.6775 | 0.3281 | 0.6894 | 0.1642 | 0.0000 | 0.8202 | 1526/0/4948 |
...
The implemented adaptation is a binary SAT/low-side baseline, so we report two output mappings for the same cached SPUR decisions.
The original `DSAT -> 3` mapping gives it some Neutral recall but no DSAT predictions.
The alternative `DSAT -> 2` mapping gives it non-zero DSAT F1, but no Neutral predictions.
Therefore, the main limitation is not only the score mapping but also the binary nature of this SPUR adaptation.
...
> For SPUR, we report both `DSAT -> 3` and `DSAT -> 2` mappings for the same binary SPUR decisions.
> The former can only recover Neutral-side cases, while the latter can recover some DSAT cases but cannot predict Neutral, confirming that this SPUR adaptation is not a full trinary predictor.
