...
merge_prompts = ["""请根据以下用户和 LLM 的对话判断用户对于 LLM 最新一次回答是否满意（若满意则输出 5），如果不满意则为什么，以下为不够满意的可能原因：
- **不够细致（0）**：方案细节不够具体，预期获得更具体的描述；
- **不满足需求（1）**：方案内容与前文需求不符合，存在错误理解需求的问题；
- **不可用（2）**：结果不具有现实可实现性，过于理想或不符合实际情况；
- **不够多样（3）**：方案种类不够多样，预期获得更多的选择；
...
请根据上述对话与原则，判断用户对于 LLM 最新一轮回答是否满意及为什么不满意（0/1/2/3/4/5），注意不要输出除了 0/1/2/3/4/5 之外的其他字符："""]
...
reflect_prompts = ["""以下是用户和 LLM 的对话历史：
{context}

用户对你你最新一轮的回答感到不够满意，你回答中的内容可能存在以下问题：
{reason}

请根据原因进行分析，并给出改进建议，注意不要输出除了改进建议之外的其他内容："""]

refine_prompts = ["""以下是用户和 LLM 的对话历史：
{context}

用户对你你最新一轮的回答感到不够满意，以下是可供参考的改进建议：
{suggestion}

请根据建议进行分析，并给出改进后的回答，注意不要输出除了改进后的回答之外的其他内容："""]

compare_prompts = ["""以下是用户和 LLM 的对话历史：
{context}

以下包含最新一轮中 LLM 给出的两个回答，请比较这两个回答的优劣，并给出你认为更好的回答（1/2），注意不要输出除了更好的回答之外的其他内容（即只输出1/2）。
