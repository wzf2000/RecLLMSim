...
class SatisfactionModel(nn.Module):
    def __init__(self, backbone: PreTrainedModel, num_reason_classes: int, alpha: float = 1.0, beta: float = 0.5, gamma: float = 0.1):
        super().__init__()
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, labels: torch.Tensor | None = None, reason_labels: torch.Tensor | None = None) -> dict[str, torch.Tensor | None]:
        ...
        if labels is not None:
            loss = self.alpha * self.satisfaction_loss_fn(ordinal_logtis, labels.float())
