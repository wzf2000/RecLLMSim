...
def get_USS_data(split: str, sample: bool = False, training: bool = True, binary: bool = False) -> tuple[list[dict], dict[int, str]] | tuple[list[dict], list[dict]]:
    ...
    for data in data_list:
        for i, utt in enumerate(data):
            if utt['role'] != 'USER':
                continue
            avg_satisfaction = np.mean(utt['satisfaction'])
            gt = int(avg_satisfaction)
            return_data_list.append({
                'history': conv_format(data[:i]),
                'user_response': utt['text'],
                'satisfaction': avg_satisfaction,
                'ground_truth': (0 if avg_satisfaction < 3 else 1) if binary else gt
            })
    print(f"Total data: {len(return_data_list)}")
    train_data, test_data = train_test_split(return_data_list, test_size=0.2, random_state=42)
    if sample:
        test_data = test_data[:100]
    if training:
        return test_data, train_data
