...
def conv_format(history: list[dict[str, str]], content_field: str | dict[str, str] = 'content') -> str:
    text = ''
    for turn in history:
        if turn['role'] == 'user' or turn['role'] == 'USER':
            if isinstance(content_field, dict):
                text += f'User: {turn[content_field["user"]]}\n\n'
            else:
                text += f'User: {turn[content_field]}\n\n' if content_field in turn else f'User: {turn["text"]}\n\n'
        else:
            if isinstance(content_field, dict):
                text += f'System: {turn[content_field["assistant"]]}\n\n'
            else:
...
职业背景：{profile['background'] if 'background' in profile else '未知'}
