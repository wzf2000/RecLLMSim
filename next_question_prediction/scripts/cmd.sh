  ...
  # 枚举数据增强比例
    for ratio in 0.1 0.2 0.5 1.0; do
      ...
      if [ "$aug_method" = "none" ]; then
        echo "python next_intent_prediction.py -t aug_$ratio ml -m $model"
        python next_intent_prediction.py -t aug_$ratio ml -m $model
      else
        echo "python next_intent_prediction.py -t aug_${aug_method}_$ratio ml -m $model"
        python next_intent_prediction.py -t aug_${aug_method}_$ratio ml -m $model
      fi
    done
  done
done
